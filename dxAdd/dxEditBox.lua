...
local font = dxCreateFont("font/font.ttf", 11, false, "proof")

function dxDrawEditBox(text, x, y, w, h, password, maxCharacter, element)
	local getText = getElementData(element, "TextEdit") or setElementData(element, "TextEdit", "")
	...
	if (#getElementData(element, "TextEdit") == 0) then
		dxDrawText(text, x + 5, y, x + w - 10, y + h, tocolor(100, 100, 100, 255), 1, font, "center", "center", true, false, false)
	end
	if (dxGetTextWidth(password and string.gsub(getElementData(element, "TextEdit"), ".", "•") or getElementData(element, "TextEdit"), 1, font) <= w - 10) then
		dxDrawText(password and string.gsub(getElementData(element, "TextEdit"), ".", "•") or getElementData(element, "TextEdit"), x + 5, y, x + w - 10, y + h, tocolor(100, 100, 100, 255), 1, font, "left", "center", true, false, false)
	else
		dxDrawText(password and string.gsub(getElementData(element, "TextEdit"), ".", "•") or getElementData(element, "TextEdit"), x + 5, y, x + w - 10, y + h, tocolor(100, 100, 100, 255), 1, font, "right", "center", true, false, false)
	end
	if (getElementData(element, "state") == true) then
		if (dxGetTextWidth(password and string.gsub(getElementData(element, "TextEdit"), ".", "•") or getElementData(element, "TextEdit"), 1, font) <= w - 10) then
			dxDrawLine(x + 5 + dxGetTextWidth(password and string.gsub(getElementData(element, "TextEdit"), ".", "•") or getElementData(element, "TextEdit"), 1, font), y + 5, x + 5 + dxGetTextWidth(password and string.gsub(getElementData(element, "TextEdit"), ".", "•") or getElementData(element, "TextEdit"), 1, font), y + h - 5, tocolor(255, 255, 255, math.abs(math.sin(getTickCount() / 255) * 255)), 1, false)
		else
...
function dxClickElement(button, state, cx, cy)
	...
	if (button == "left") and (state == "down") then
		...
		for id, element in ipairs(getElementsByType("CMR:EditBox:Celular")) do
			if (getElementData(element, "mouseState") == "hovered") then
				editBoxId = id
		...
		if (editBoxId) then
			if (isElement(getElementsByType("CMR:EditBox:Celular")[editBoxId])) then
				if (getElementData(getElementsByType("CMR:EditBox:Celular")[editBoxId], "state") == false) then
					guiSetInputMode("no_binds")
					setElementData(getElementsByType("CMR:EditBox:Celular")[editBoxId], "state", true)
				end
			end
		end
	end
...
function dxCharacterElement(button)
	...
	for _, element in ipairs(getElementsByType("CMR:EditBox:Celular")) do
		if (getElementData(element, "state") == true) then
			if (#getElementData(element, "TextEdit") < getElementData(element, "maximum")) then
				setElementData(element, "TextEdit", getElementData(element, "TextEdit")..button)
			end
		end
	end
end
...
function dxKeyElement(button, press)
	...
	if (press) and (button == "backspace") then
		for _, element in ipairs(getElementsByType("CMR:EditBox:Celular")) do
			if (getElementData(element, "state") == true) then
				if (#getElementData(element, "TextEdit") > 0) then
					setElementData(element, "TextEdit", string.sub(getElementData(element, "TextEdit"), 1, #getElementData(element, "TextEdit") - 1))
				end
			end
		end
	end
